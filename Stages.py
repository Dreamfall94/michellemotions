...
def secondStage(michelle, interlocutor, emo1stage):
    ...
    level2 = a * 10
    level3 = a * 16
    return {'1st level of emotion':level1, '2nd level of emotion':level2, '3rd level of emotion':level3}
...
if __name__ == '__main__':
    ...
    vocabulary = firstStage(michelle, lena)
    emo1stage = max(vocabulary, key=vocabulary.get)
    print(emo1stage)
    vocabulary = secondStage(michelle, lena, emo1stage)
    emo2stage = max(vocabulary, key=vocabulary.get)
    print(emo2stage)
    # [0..7] = f1(l,m) = b
